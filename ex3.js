...
let nuevoArray = numeros.map((e) => e - 1);
...
let nuevosAnimales = animales.map((e) => e.toUpperCase());
...
let pacientesVisitados = pacientes.map((e) => e.ultimaVisita.toLocaleString());
...
let pacientesAvisos = pacientes.map(e => ({...e, avisos: e.ultimaVisitaPagada ? " " : "Paciente moroso, cobrar!"})); 
...
let pacientesLista = pacientes.map(e => {return `<li>${e.nombre}</li>`});
